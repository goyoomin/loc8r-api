...
const reviewsCreate = async (req, res) => {
  ...
  if (!locationId) return res.status(404).json({ message: 'Location not found' });

  try {
    const location = await Loc.findById(locationId).select('reviews').exec();
    if (location) await doAddReview(req, res, location);
    else return res.status(404).json({ message: 'Location not found' });
  } catch (err) {
    return res.status(400).json(err);
  }
};
...
const reviewsDeleteOne = async (req, res) => {
  ...
  try {
    ...
    if (location.reviews && location.reviews.length > 0) {
      ...
      return res.status(204).json(null); // No Content
    } else {
